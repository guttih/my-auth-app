...
[source,bash]
----
find . \( -path './node_modules' -o -path './.next' -o -path './.git'  -o -path './public' -o -path './prisma/migrations'   \) -prune -o -print
----
...
.Open a terminal and connect to the PostgreSQL database myauthdb:
[source,bash]
 sudo -u postgres psql -d myauthdb
